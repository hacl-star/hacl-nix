...
*/
{ stdenv, lib, makeWrapper, installShellFiles, which, z3, ocamlPackages, sphinx
, python39Packages, perl, ... }:
let
  ...
  */
  ...
  binary-of-ml-snapshot =
    { src, pname ? src.pname, version ? src.version, opts ? defaults }:
    let
      pkg = stdenv.mkDerivation (defaults // opts // {
        inherit pname version src;

        nativeBuildInputs = [ makeWrapper installShellFiles z3 perl ]
        ...
        installPhase = ''
          ...
          copyBin () { cp "bin/$1" $out/bin
                       ...
                       # As a workaround, we just replace its with `a`s
                       perl -pi -e "s/.nix.store.${
                         lib.head (builtins.match ".*/([0-9a-z]{32}).*"
                           (toString ocamlPackages.ocaml))
                       }/aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa/g" "$out/bin/$1"
...
in {
  inherit binary-of-fstar ml-extraction-of-fstar binary-of-ml-snapshot
    check-fstar with-existing-fstar ocamlBuildInputs ocamlNativeBuildInputs;
